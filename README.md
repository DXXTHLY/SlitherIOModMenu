...
(function() {
    ...
    const state = {
        ...
        extraFeatures: [
            ...
            {name: "X-Ray Vision", desc: "See Snake Score", key: "t"},
            {name: "Auto Split", desc: "More for Agario", key: "y"},
...
(function() {
    ...
    const config = {
        ...
        featureKeys: ["1","2","3","4","5","6","7","8","9","0"],
        quickBoostKey: " ",
        extraKeys: ["q","w","e","r","t","y","u","i","o","p","k"]
    };
    ...
    const state = {
        ...
        fpsCounter: 0,
        currentFps: 0,
        cursorRadius: config.cursorConfinement.defaultRadius,
        ...
        extraFeatures: [
            ...
            {name: "Mini Mode", desc: "Shrink your snake size", key: "o"},
            {name: "Freeze Mode", desc: "Freeze nearby snakes", key: "p"},
            {name: "Cursor Confinement", desc: "Keep cursor within a circle", key: "k"}
        ]
    };
    ...
    function init() {
        ...
        // Setup cursor confinement
        setupCursorConfinement();

        // Update center position on resize
        window.addEventListener('resize', updateCenterPosition);

        // Check for death screen periodically
        setInterval(checkDeathScreen, 100);
    ...
    function updateCenterPosition() {
        ...
        if (state.features.cursorConfinement) {
            drawConfinementCircle();
        }
    }

    function setupCursorConfinement() {
        // Create canvas for confinement circle
        state.elements.confinementCanvas = document.createElement('canvas');
        state.elements.confinementCanvas.width = window.innerWidth;
        state.elements.confinementCanvas.height = window.innerHeight;
        state.elements.confinementCanvas.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
            pointer-events: none;
            z-index: 9998;
        `;
        document.body.appendChild(state.elements.confinementCanvas);
    ...
    function handleMouseMove(e) {
        if (!state.features.cursorConfinement) return;

        const dx = e.clientX - state.centerX;
        const dy = e.clientY - state.centerY;
        const distance = Math.sqrt(dx * dx + dy * dy);

        if (distance > state.cursorRadius) {
            const angle = Math.atan2(dy, dx);
            const newX = state.centerX + Math.cos(angle) * state.cursorRadius;
            const newY = state.centerY + Math.sin(angle) * state.cursorRadius;

            // Move cursor to edge of circle
    ...
    function drawConfinementCircle() {
        if (!state.features.cursorConfinement || !state.elements.confinementCanvas) return;

        const ctx = state.elements.confinementCanvas.getContext('2d');
        ctx.clearRect(0, 0, state.elements.confinementCanvas.width, state.elements.confinementCanvas.height);

        ctx.beginPath();
        ctx.arc(state.centerX, state.centerY, state.cursorRadius, 0, Math.PI * 2);
        ctx.fillStyle = config.cursorConfinement.circleColor;
        ctx.fill();
        ctx.lineWidth = config.cursorConfinement.borderWidth;
        ctx.strokeStyle = config.cursorConfinement.borderColor;
        ctx.stroke();
    }

    function removeConfinementCircle() {
        if (state.elements.confinementCanvas) {
            const ctx = state.elements.confinementCanvas.getContext('2d');
            ctx.clearRect(0, 0, state.elements.confinementCanvas.width, state.elements.confinementCanvas.height);
        }
    }
    ...
    function checkDeathScreen() {
        const deathScreen = document.querySelector('.snakeButton, .btnPlayAgain, .playAgain');
        const isDeathScreenVisible = !!deathScreen;

        if (isDeathScreenVisible && !state.deathScreenVisible && state.features.deathSound) {
            // Death screen just appeared
            state.deathAudio.volume = config.deathVolume;
            state.deathAudio.currentTime = 0;
            state.deathAudio.play().catch(e => console.log("Couldn't play death sound:", e));
            state.wasAlive = false;
        } else if (!isDeathScreenVisible && !state.wasAlive) {
            state.wasAlive = true;
        }

        state.deathScreenVisible = isDeathScreenVisible;
    }
    ...
    function updateFeatures(deltaTime) {
        ...
        // Death sound
        if (state.features.deathSound) checkDeathScreen();
    ...
    function updateMenu() {
        let extraFeaturesHtml = '';
        state.extraFeatures.forEach((feat, idx) => {
            const featureName = feat.name.replace(/\s+/g, '').toLowerCase();
            const isActive = state.features[featureName];
            extraFeaturesHtml += `
                <p><strong>${feat.key.toUpperCase()}. ${feat.name}:</strong>
                <span style="color:${isActive?'lime':'red'}">${isActive?'ON':'OFF'}</span>
                <br><small style="color:#aaa">${feat.desc}</small></p>
        ...
        // Add cursor confinement controls if the feature exists
        const cursorConfinementIndex = state.extraFeatures.findIndex(f => f.name === "Cursor Confinement");
        if (cursorConfinementIndex >= 0) {
            const feature = state.extraFeatures[cursorConfinementIndex];
            const featureName = feature.name.replace(/\s+/g, '').toLowerCase();
            const isActive = state.features[featureName];

            extraFeaturesHtml += `
                <div style="margin-top:10px;${isActive?'':'display:none'}" id="cursorConfinementControls">
                    <label style="color:#aaa">Circle Radius: ${state.cursorRadius.toFixed(0)}px</label>
                    <input type="range" min="${config.cursorConfinement.minRadius}" max="${config.cursorConfinement.maxRadius}"
                        value="${state.cursorRadius}" step="10" style="width:100%"
                        oninput="updateCursorRadius(this.value)" onchange="updateCursorRadius(this.value)">
                </div>
            `;
        }
        ...
        state.elements.menu.innerHTML = `
            ...
            <script>
                function updateCursorRadius(value) {
                    window.postMessage({
                        type: 'UPDATE_CURSOR_RADIUS',
                        radius: parseFloat(value)
                    }, '*');
                    document.querySelector('#cursorConfinementControls label').textContent = 'Circle Radius: ' + Math.round(value) + 'px';
                }
            </script>
        `;
    }
    ...
    // Add message listener for radius updates from the menu
    window.addEventListener('message', function(event) {
        if (event.data.type === 'UPDATE_CURSOR_RADIUS') {
            state.cursorRadius = event.data.radius;
            drawConfinementCircle();
        }
    });
...
(function () {
    ...
    function updateMenu() {
        menu.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
                <h2 style="margin:0;color:#4CAF50;">ULTIMATE SLITHER.IO MOD</h2>
                <div style="color:#aaa;font-size:12px">v7.7</div>
            </div>
    ...
    function applyPerformanceMode() {
        // Slither.io visuals are not public API, but we can try to tweak some global variables
        // These are best-effort, may not work on all clones
        if (typeof window.visuals === "object") {
            // If a mod exposes a visuals object, use it
            switch (state.features.performanceMode) {
                case 1: // Low
                    window.visuals.snakeShadows = false;
                    window.visuals.foodShadows = false;
                    window.visuals.background = false;
                    break;
                case 2: // Medium
                    window.visuals.snakeShadows = false;
                    window.visuals.foodShadows = true;
                    window.visuals.background = false;
                    break;
                case 3: // High
                    window.visuals.snakeShadows = true;
                    window.visuals.foodShadows = true;
                    window.visuals.background = true;
                    break;
                case 4: // Ultra
                    window.visuals.snakeShadows = true;
                    window.visuals.foodShadows = true;
